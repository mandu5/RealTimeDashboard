...
- **임계값 (threshold)**: 모델 학습 시 결정 경계 `offset_` 사용 (기본 -0.3). `score < threshold` 이면 이상으로 판정.
- **3D/타임라인 표시**: Y축이 이상 점수. 빨간 구간 = score < threshold.

#### 신뢰도 (Confidence) 계산

- **공식**: `confidence = sigmoid((threshold - score) / spread)`
  - `threshold`: 모델 `offset_` (학습 데이터 기반 결정 경계)
  - `spread`: `max(|threshold|×0.5, 0.05)` — 경계 주변 스무딩
- **의미**: **"이상으로 판정할 확신의 정도"** (0%: 정상, 100%: 확실한 이상, 50%: 경계 구간)
...
| 구간    | 색상 | 의미                        |
| ------- | ---- | --------------------------- |
| 0–30%   | 초록 | 정상 또는 경미한 이상       |
| 30–70%  | 노랑 | 경계 구간, 주의 필요        |
| 70–100% | 빨강 | 확실한 이상, 즉시 확인 권장 |

#### 기여 특성 (Contributing Features)

- **출처**: StandardScaler 출력 = Z-score. 학습 시 평균·표준편차로 정규화.
...
- **축**: X=지터(ms), Y=PPS, Z=손실률(%) — 원시 특성 3개, PCA 미사용.
- **분류 기준**: 이상(score < threshold, 빨강), 경고(threshold ≤ score < threshold+0.1, 노랑), 정상(파랑).
- **마커 크기**: 이상일수록 큼 (7~18), 정상 4.

#### 이상 타임라인

- **Y축**: 이상 점수 (score), 범위 -1 ~ 0.5.
- **임계선**: threshold(예: -0.3) 수평 점선.
- **이상 구간**: score < threshold 인 연속 구간을 빨간 배경으로 표시.
