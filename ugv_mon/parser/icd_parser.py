...
class ICDParser:
    ...
    def parse(self, data: bytes) -> ParseResult:
        ...
        # 페이로드 파싱 (상태보고)
        payload = None
        if header.msg_code == MSG_CODE_STATUS_REPORT:
            payload_data = data[ICD_HEADER_SIZE:-ICD_CHECKSUM_SIZE]
            try:
    ...
    def _verify_checksum(self, data: bytes) -> bool:
        ...
        if len(data) < ICD_CHECKSUM_SIZE:
            return False
        data_without_checksum = data[:-ICD_CHECKSUM_SIZE]
    ...
    def _parse_status_payload(self, payload: bytes) -> StatusPayload:
        ...
        emergency_status = {
            ...
            for i, name in enumerate(EMERGENCY_LABELS)
        }
        ...
        return StatusPayload(
            ...
            operation_mode_label=OPERATION_MODE_LABELS.get(operation_mode, f"알 수 없음 ({operation_mode})"),
            authority=authority,
            authority_label=AUTHORITY_LABELS.get(authority, f"알 수 없음 ({authority})"),
            driving_state=driving_state,
            driving_state_label=DRIVING_STATE_LABELS.get(driving_state, f"알 수 없음 ({driving_state})"),
