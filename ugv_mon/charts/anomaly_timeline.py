...
def create_anomaly_timeline(
    ...
    fig.update_layout(
        ...
        height=280,
        margin=dict(l=50, r=20, t=50, b=70),  # 하단 여백 증가
        showlegend=False,
    )
...
def _create_empty_timeline() -> go.Figure:
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="시간", nticks=10),
        yaxis=dict(title="이상 점수", range=[-1, 0.5]),
        paper_bgcolor="rgba(17, 24, 39, 1)",
        plot_bgcolor="rgba(17, 24, 39, 0.8)",
        font=dict(color="white", size=10),
        height=280,
...
def create_confidence_gauge(confidence: float) -> go.Figure:
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge=dict(
            ...
            borderwidth=0,
            steps=[
                dict(range=[0, 30], color="rgba(34, 197, 94, 0.3)"),
        ...
        title=dict(text="이상 신뢰도", font=dict(color="white", size=14)),
    ))
